...
uint8 INIT_CHOICE = -1;
uint8 QUIT_CHOICE = 12;

uint32 handleMenu(uint32 choice, LinkedList *list) {
	printf("\nYour choice: %ld\n", choice);
	if (choice == QUIT_CHOICE || choice == 0) return 0;
	...
	if (choice != INIT_CHOICE) {
		...
		MethodType methods[] = { addStudent, displayStudents, searchStudentsById };
	...
	puts("7. Calculate Average GPA");
	puts("8. Find Student with Highest GPA");
	puts("9. Find Student with Lowest GPA");
	puts("12. Quit / Exit Program");
	puts("===================================\n");
